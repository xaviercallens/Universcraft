"""
UniversCraft / HoloEngine — Real-Time Demonstration of S² Earth Orbit Manifold
Renders a 360-degree rotational orbit in real time on the NVIDIA GeForce RTX GPU.
Saves frames and compiles an animated demonstration.
"""
...
def main():
    print("=" * 75)
    print(" 🌍 REAL-TIME TOPOLOGY DEMONSTRATION: S² SPHERICAL MANIFOLD ORBIT")
    print("=" * 75)

    base_dir = os.path.dirname(os.path.abspath(__file__))
    shader_path = os.path.join(base_dir, "assets", "shaders", "scene_earth_orbit.wgsl")
    ...
    print(f"[*] Dispatching {num_frames} orbital camera frames at {width}x{height} resolution...")
    ...
    for f_idx in range(num_frames):
        ...
        cam_y = orbit_height + 4.0 * math.sin(angle * 2.0)
    ...
    # Save animated GIF
    out_gif = os.path.join(base_dir, "public", "output", "earth_orbit_realtime_demo.gif")
    # Resize slightly for compact animated GIF (e.g. 640x360)
    print(f"[*] Compiling orbital demonstration GIF to: {out_gif}")
    small_frames = [f.resize((640, 360), Image.Resampling.BILINEAR) for f in frames]
    small_frames[0].save(
        out_gif,
        save_all=True,
        append_images=small_frames[1:],
        duration=50, # 20 FPS playback
        loop=0
    )
